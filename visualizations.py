...
COLOR_CRITICAL_RED = "#E53E3E"; COLOR_WARNING_AMBER = "#F59E0B"; COLOR_POSITIVE_GREEN = "#10B981"; COLOR_INFO_BLUE = "#3B82F6"; COLOR_NEUTRAL_GRAY = "#6B7280"; COLOR_ACCENT_INDIGO = "#4F46E5"
COLOR_WHITE_TEXT = "#FFFFFF"; COLOR_LIGHT_TEXT = "#EAEAEA"; COLOR_DARK_TEXT_ON_LIGHT_BG_HC = "#000000"; COLOR_SUBTLE_GRID_STD = "#374151"; COLOR_SUBTLE_GRID_HC = "#555555"; COLOR_AXIS_LINE_STD = "#4A5568"; COLOR_AXIS_LINE_HC = "#777777"
PLOTLY_TEMPLATE = "plotly_dark"; EPSILON = 1e-9
...
def _apply_common_layout_settings(fig, title_text, high_contrast=False, yaxis_title=None, xaxis_title="Time Step (Interval)", yaxis_range=None, show_legend=True):
    font_color = COLOR_WHITE_TEXT if high_contrast else COLOR_LIGHT_TEXT; title_font_color = COLOR_WHITE_TEXT; grid_color = COLOR_SUBTLE_GRID_HC if high_contrast else COLOR_SUBTLE_GRID_STD; axis_line_color = COLOR_AXIS_LINE_HC if high_contrast else COLOR_AXIS_LINE_STD; legend_bgcolor = "rgba(0,0,0,0.7)" if high_contrast else "rgba(31, 41, 55, 0.7)"; legend_border_color = COLOR_NEUTRAL_GRAY
    fig.update_layout(template=PLOTLY_TEMPLATE, title=dict(text=title_text, x=0.5, font=dict(size=16, color=title_font_color)), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color=font_color, size=11), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, bgcolor=legend_bgcolor, bordercolor=legend_border_color, borderwidth=0.5, font_size=10, traceorder="normal", font=dict(color=font_color)) if show_legend else None, margin=dict(l=60, r=40, t=70, b=60), xaxis=dict(title=xaxis_title, gridcolor=grid_color, zerolinecolor=grid_color, showline=True, linewidth=1, linecolor=axis_line_color, rangemode='tozero' if xaxis_title and "Time" in xaxis_title else 'normal', titlefont=dict(size=13, color=font_color), tickfont=dict(size=11, color=font_color)), yaxis=dict(title=yaxis_title, gridcolor=grid_color, zerolinecolor=grid_color, showline=True, linewidth=1, linecolor=axis_line_color, range=yaxis_range, titlefont=dict(size=13, color=font_color), tickfont=dict(size=11, color=font_color)), hovermode="x unified", dragmode='pan')

def _get_no_data_figure(title_text, high_contrast=False):
    fig = go.Figure(); _apply_common_layout_settings(fig, title_text, high_contrast, show_legend=False); font_color = COLOR_WHITE_TEXT if high_contrast else COLOR_LIGHT_TEXT; fig.add_annotation(text="No data available for this visualization.", showarrow=False, font=dict(size=14, color=font_color)); return fig
def _get_no_data_pie_figure(title_text, high_contrast=False):
    fig = go.Figure(); font_color = COLOR_WHITE_TEXT if high_contrast else COLOR_LIGHT_TEXT; fig.update_layout(title=dict(text=title_text, x=0.5, font=dict(size=16, color=font_color)), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color=font_color, size=11), annotations=[dict(text="No data available for this visualization.", showarrow=False, xref="paper", yref="paper", x=0.5, y=0.5, font=dict(size=14, color=font_color))]); return fig

def plot_key_metrics_summary(compliance: float, proximity: float, wellbeing: float, downtime: float, target_compliance: float, target_proximity: float, target_wellbeing: float, target_downtime: float, high_contrast: bool = False, color_positive: str = COLOR_POSITIVE_GREEN, color_warning: str = COLOR_WARNING_AMBER, color_negative: str = COLOR_CRITICAL_RED, accent_color: str = COLOR_ACCENT_INDIGO):
    figs = []; font_color_gauge = COLOR_WHITE_TEXT if high_contrast else COLOR_LIGHT_TEXT; gauge_base_bgcolor = "rgba(42, 52, 71, 0.7)" if not high_contrast else "rgba(17,17,17,0.7)"
    metrics_config = [("Task Compliance", compliance, target_compliance, accent_color, "%", False), ("Collaboration Idx", proximity, target_proximity, accent_color, "%", False), ("Well-Being Idx", wellbeing, target_wellbeing, accent_color, "%", False), ("Total Downtime", downtime, target_downtime, accent_color, " min", True)]
    for title, raw_value, raw_target, bar_color_val, suffix, lower_is_better in metrics_config:
        fig = go.Figure(); value = 0.0; target = 0.0
        try: value = float(raw_value) if raw_value is not None and pd.notna(raw_value) else 0.0
        except (ValueError, TypeError): logger.warning(f"Gauge '{title}': Invalid raw_value '{raw_value}'. Defaulting to 0.0."); value = 0.0
        try: target = float(raw_target) if raw_target is not None and pd.notna(raw_target) else 0.0
        except (ValueError, TypeError): logger.warning(f"Gauge '{title}': Invalid raw_target '{raw_target}'. Defaulting to 0.0."); target = 0.0
        logger.debug(f"Gauge '{title}': Value={value}, Target={target}, Suffix='{suffix}', LowerIsBetter={lower_is_better}")
        increasing_is_good_color = color_positive if not lower_is_better else color_negative; decreasing_is_good_color = color_negative if not lower_is_better else color_positive
        base_max_val = 10.0; axis_max_val = 100.0 if suffix == "%" else max(value * 1.25, target * 1.5, base_max_val)
        ...
        steps_config = []; 
        if lower_is_better: 
            s1_upper = target; s2_upper = target * 1.25 if target > EPSILON else axis_max_val / 2.0 
            if s1_upper < 0: s1_upper = 0; 
            if s2_upper < s1_upper: s2_upper = s1_upper + EPSILON
            if target == 0.0 : s1_upper = EPSILON; s2_upper = axis_max_val / 2.0 if axis_max_val > EPSILON else EPSILON * 2
            steps_config = [{'range': [0, s1_upper], 'color': color_positive}, {'range': [s1_upper, s2_upper], 'color': color_warning}, {'range': [s2_upper, axis_max_val], 'color': color_negative}]
        else: 
            s1_upper = target * 0.8 if target > EPSILON else axis_max_val / 3.0; s2_upper = target       
            if s1_upper >= s2_upper and target > EPSILON: s1_upper = target * 0.7 
            if s1_upper < 0: s1_upper = 0
            if s2_upper < s1_upper: s2_upper = s1_upper + EPSILON
            steps_config = [{'range': [0, s1_upper], 'color': color_negative}, {'range': [s1_upper, s2_upper], 'color': color_warning}, {'range': [s2_upper, axis_max_val], 'color': color_positive}]
        valid_steps = []; current_lower_bound = 0.0
        for i, step in enumerate(steps_config):
            s_lower, s_upper = step['range']; s_lower = float(s_lower) if pd.notna(s_lower) else current_lower_bound; s_upper = float(s_upper) if pd.notna(s_upper) else axis_max_val
            s_lower = max(current_lower_bound, s_lower); s_upper = min(max(s_lower + EPSILON, s_upper), axis_max_val) 
            if s_upper > s_lower: valid_steps.append({'range': [s_lower, s_upper], 'color': step['color']}); current_lower_bound = s_upper
            elif i == len(steps_config) - 1 and not valid_steps: valid_steps.append({'range': [0, axis_max_val], 'color': accent_color})
        if not valid_steps: logger.error(f"Gauge '{title}': All steps invalid. Fallback."); valid_steps = [{'range': [0, axis_max_val], 'color': accent_color}]
        logger.debug(f"Gauge '{title}': axis_max_val={axis_max_val}, valid_steps={valid_steps}")
        fig.add_trace(go.Indicator(mode="gauge+number+delta", value=value, delta={'reference': target, 'increasing': {'color': increasing_is_good_color}, 'decreasing': {'color': decreasing_is_good_color}, 'font': {'size': 12}}, title={'text': title, 'font': {'size': 12, 'color': font_color_gauge}}, number={'suffix': suffix, 'font': {'size': 18, 'color': font_color_gauge}}, gauge={'axis': {'range': [0, axis_max_val], 'tickwidth': 1, 'tickcolor': COLOR_NEUTRAL_GRAY if not high_contrast else COLOR_WHITE_TEXT}, 'bar': {'color': bar_color_val if bar_color_val else accent_color, 'thickness': 0.65}, 'bgcolor': gauge_base_bgcolor, 'borderwidth': 0.5, 'bordercolor': COLOR_NEUTRAL_GRAY, 'steps': valid_steps, 'threshold': {'line': {'color': font_color_gauge, 'width': 2.5}, 'thickness': 0.8, 'value': target}}))
        fig.update_layout(height=180, margin=dict(l=10,r=10,t=30,b=10), paper_bgcolor="rgba(0,0,0,0)", font=dict(color=font_color_gauge)); figs.append(fig)
    return figs
...
def plot_task_compliance_score(data, disruption_points=None, forecast_data=None, z_scores=None, high_contrast=False):
    ...
    cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; fig = go.Figure(); x_vals = list(range(len(data)))
    ...
    if disruption_points:
        disruption_line_color = COLOR_WARNING_AMBER; disruption_annot_font_color = COLOR_WHITE_TEXT if high_contrast else COLOR_DARK_TEXT_ON_LIGHT_BG_HC; disruption_annot_bgcolor = "rgba(245,158,11,0.3)" if not high_contrast else "rgba(245,158,11,0.6)"
    ...
    min_val_list = [v for v in data if v is not None and pd.notna(v)]; max_val_list = [v for v in data if v is not None and pd.notna(v)]
    if forecast_data: min_val_list.extend([v for v in forecast_data if v is not None and pd.notna(v)]); max_val_list.extend([v for v in forecast_data if v is not None and pd.notna(v)])
    min_val = min(min_val_list) if min_val_list else 0.0; max_val = max(max_val_list) if max_val_list else 100.0
...
def plot_collaboration_proximity_index(data, disruption_points=None, forecast_data=None, high_contrast=False):
    ...
    cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; fig = go.Figure(); x_vals = list(range(len(data)))
    fig.add_trace(go.Scatter(x=x_vals, y=data, mode='lines+markers', name='Collab. Index', line=dict(color=cat_palette[1] if len(cat_palette)>1 else cat_palette[0], width=2.5), marker=dict(size=5, symbol="diamond"), hovertemplate='Collab. Index: %{y:.1f}%<extra></extra>'))
    if forecast_data and len(forecast_data)==len(data): fig.add_trace(go.Scatter(x=x_vals, y=forecast_data, mode='lines', name='Forecast', line=dict(color=cat_palette[2] if len(cat_palette)>2 else cat_palette[0], dash='dashdot', width=2), hovertemplate='Forecast: %{y:.1f}%<extra></extra>'))
    if disruption_points:
        disruption_line_color = COLOR_WARNING_AMBER; disruption_annot_font_color = COLOR_WHITE_TEXT if high_contrast else COLOR_DARK_TEXT_ON_LIGHT_BG_HC; disruption_annot_bgcolor = "rgba(245,158,11,0.3)" if not high_contrast else "rgba(245,158,11,0.6)"
        for dp_step in disruption_points:
...
def plot_operational_efficiency(efficiency_df, selected_metrics, disruption_points=None, high_contrast=False):
    ...
    cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; fig = go.Figure()
...
def plot_worker_distribution(team_positions_df, facility_size, config, use_3d=False, selected_step=0, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    facility_width, facility_height = facility_size; cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE
    zone_names = df_step['zone'].unique(); zone_color_map = {zone: cat_palette[i % len(cat_palette)] for i, zone in enumerate(zone_names)}
    status_symbols_map = {"working": "circle", "idle": "square", "break":"diamond", "fatigued": "cross", "exhausted": "x-thin", "disrupted":"hourglass"}; df_step['symbol_plotly'] = df_step['status'].map(status_symbols_map).fillna("circle")
    if use_3d:
        ...
        grid_c = COLOR_SUBTLE_GRID_HC if high_contrast else COLOR_SUBTLE_GRID_STD; fig.update_layout(xaxis_showgrid=True, yaxis_showgrid=True, xaxis_gridcolor=grid_c, yaxis_gridcolor=grid_c, xaxis_zeroline=True, yaxis_zeroline=True, xaxis_zerolinecolor=grid_c, yaxis_zerolinecolor=grid_c)
    ...
    font_c = COLOR_WHITE_TEXT if high_contrast else COLOR_LIGHT_TEXT; ee_point_color = COLOR_INFO_BLUE; work_area_line_color = cat_palette[len(zone_names) % len(cat_palette)] if cat_palette else COLOR_INFO_BLUE 
    if show_entry_exit and 'ENTRY_EXIT_POINTS' in config:
        for point in config['ENTRY_EXIT_POINTS']: shapes.append(go.layout.Shape(type="circle", x0=point['coords'][0]-1.5, y0=point['coords'][1]-1.5, x1=point['coords'][0]+1.5, y1=point['coords'][1]+1.5, fillcolor=ee_point_color, line_color=COLOR_WHITE_TEXT, line_width=1, opacity=0.9, layer="above")); annotations.append(dict(x=point['coords'][0], y=point['coords'][1]+4, text=point['name'][:2].upper(), showarrow=False, font=dict(size=9, color=font_c), borderpad=2, bgcolor="rgba(0,0,0,0.3)" if not high_contrast else "rgba(50,50,50,0.5)"))
    if show_prod_lines and 'WORK_AREAS' in config:
         for area_name, area_details in config['WORK_AREAS'].items():
            if 'coords' in area_details: 
                (x0,y0), (x1,y1) = area_details['coords']; fill_color_rgba = "rgba(128,128,128,0.05)"
                try: fill_color_rgba = f"rgba({int(work_area_line_color[1:3],16)},{int(work_area_line_color[3:5],16)},{int(work_area_line_color[5:7],16)},0.05)" if work_area_line_color.startswith("#") and len(work_area_line_color) == 7 else "rgba(128,128,128,0.05)" 
                except ValueError: pass
                shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1), line=dict(color=work_area_line_color, dash="dashdot", width=1.5), fillcolor=fill_color_rgba, layer="below")); annotations.append(dict(x=(x0+x1)/2, y=min(y0,y1)-4, text=area_name, showarrow=False, font=dict(size=9, color=COLOR_NEUTRAL_GRAY), opacity=0.9, yanchor="top"))
    _apply_common_layout_settings(fig, f"Worker Spatial Distribution (Time: {selected_step*2} min)", high_contrast, yaxis_title="Y (m)", xaxis_title="X (m)", show_legend=True); fig.update_layout(shapes=shapes, annotations=annotations, legend_title_text='Zone'); return fig
...
def plot_worker_density_heatmap(team_positions_df, facility_size, config, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    facility_width, facility_height = facility_size; heatmap_colorscale = HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES[4] if high_contrast else ACCESSIBLE_SEQUENTIAL_PLOTLY_SCALES[1]
    ...
    ee_point_color = COLOR_INFO_BLUE; cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; work_area_line_color = cat_palette[0] if cat_palette else COLOR_INFO_BLUE 
    if show_entry_exit and 'ENTRY_EXIT_POINTS' in config:
...
def plot_worker_wellbeing(scores, triggers, disruption_points=None, high_contrast=False):
    ...
    cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; fig = go.Figure(); x_vals = list(range(len(scores)))
    ...
    for trigger_type, points in triggers.items():
        ...
        if isinstance(points, list): flat_points = points
            ...
            all_wa_points = set(); [all_wa_points.update(p_list) for p_list in points.values() if isinstance(p_list, list)]; flat_points = list(all_wa_points); processed_trigger_type = 'work_area_general'
        ...
        if valid_points: 
            ...
            if y_values_for_markers: fig.add_trace(go.Scatter(x=valid_points_for_trace, y=y_values_for_markers, mode='markers', name=style['name'], marker=dict(color=style['color'], size=10, symbol=style['symbol'], line=dict(width=1.5, color=COLOR_WHITE_TEXT)), hovertemplate=f'{style["name"]}: %{{y:.1f}}% at Step %{{x}}<extra></extra>'))
...
def plot_downtime_trend(downtime_events_list, interval_threshold, disruption_points=None, high_contrast=False): # Added disruption_points (though less common for bar)
    if not downtime_events_list: return _get_no_data_figure("Downtime per Interval", high_contrast)
    fig = go.Figure(); downtime_durations = [event.get('duration', 0.0) for event in downtime_events_list if isinstance(event,dict)]; x_vals = list(range(len(downtime_durations)))
    ...
    max_y_val = 10.0 
    if downtime_durations: max_duration = max(downtime_durations) if downtime_durations else 0; max_y_val = max(max_duration * 1.15, interval_threshold * 1.5, 10.0)
    else: max_y_val = max(interval_threshold * 1.5, 10.0)
...
def plot_downtime_causes_pie(downtime_events_list, high_contrast=False):
    if not downtime_events_list: return _get_no_data_pie_figure("Downtime Distribution by Cause", high_contrast)
    causes_summary = {}; total_downtime_duration_for_pie = 0.0
    for event in downtime_events_list:
        if not isinstance(event, dict): logger.warning(f"Skipping non-dict event: {event}"); continue
        raw_duration = event.get('duration', 0); cause = event.get('cause', 'Unknown'); current_duration_value = 0.0
        try: current_duration_value = float(raw_duration)
        except (ValueError, TypeError): logger.warning(f"Invalid duration '{raw_duration}' for cause '{cause}'. Skipping."); continue 
        if current_duration_value <= 0: continue
        ...
        if cause == "None" or cause == "Unknown": continue
    ...
    if not causes_summary: return _get_no_data_pie_figure("Downtime by Cause (No Categorized Downtime)", high_contrast)
    ...
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=.4, pull=[0.02]*num_causes, marker_colors=pie_colors, textinfo='label+percent', insidetextorientation='auto', hovertemplate="<b>Cause:</b> %{label}<br><b>Duration:</b> %{value:.1f} min<br><b>Share:</b> %{percent}<extra></extra>", sort=True, direction='clockwise')])
    fig.update_traces(textfont_size=10, marker=dict(line=dict(color=slice_border_color, width=1)))
    _apply_common_layout_settings(fig, f"Downtime Distribution by Cause (Total: {total_downtime_duration_for_pie:.0f} min)", high_contrast, show_legend=(num_causes <= 7)); return fig
