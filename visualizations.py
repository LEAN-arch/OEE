...
def plot_task_compliance_score(data, disruption_points, forecast_data=None, z_scores=None, high_contrast=False):
    ...
    disruption_annot_bgcolor = "rgba(245,158,11,0.3)" if not high_contrast else "rgba(245,158,11,0.6)" # Amber bg with adjusted opacity
...
def plot_worker_distribution(team_positions_df, facility_size, config, use_3d=False, selected_step=0, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    ee_point_color = COLOR_INFO_BLUE; work_area_line_color = cat_palette[len(zone_names) % len(cat_palette)]
    ...
    if show_prod_lines and 'WORK_AREAS' in config:
         for area_name, area_details in config['WORK_AREAS'].items():
            if 'coords' in area_details: (x0,y0), (x1,y1) = area_details['coords']; shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1), line=dict(color=work_area_line_color, dash="dashdot", width=1.5), fillcolor=f"rgba({int(work_area_line_color[1:3],16)},{int(work_area_line_color[3:5],16)},{int(work_area_line_color[5:7],16)},0.05)", layer="below")); annotations.append(dict(x=(x0+x1)/2, y=min(y0,y1)-4, text=area_name, showarrow=False, font=dict(size=9, color=COLOR_NEUTRAL_GRAY), opacity=0.9, yanchor="top"))
...
def plot_worker_density_heatmap(team_positions_df, facility_size, config, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    ee_point_color = COLOR_INFO_BLUE; cat_palette = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast else ACCESSIBLE_CATEGORICAL_PALETTE; work_area_line_color = cat_palette[0]
    if show_entry_exit and 'ENTRY_EXIT_POINTS' in config:
...
def plot_worker_wellbeing(scores, triggers, high_contrast=False):
    ...
    for trigger_type, points in triggers.items():
        ...
        if valid_points: style = trigger_styles.get(processed_trigger_type, default_trigger_style); fig.add_trace(go.Scatter(x=valid_points, y=[scores[p] for p in valid_points], mode='markers', name=style['name'], marker=dict(color=style['color'], size=10, symbol=style['symbol'], line=dict(width=1.5, color=COLOR_WHITE_TEXT)), hovertemplate=f'{style["name"]}: %{{y:.1f}}% at Step %{{x}}<extra></extra>'))
...
def plot_downtime_trend(downtime_events_list, interval_threshold, high_contrast=False):
    ...
    fig = go.Figure(); downtime_durations = [event.get('duration', 0) for event in downtime_events_list]; x_vals = list(range(len(downtime_durations)))
