...
TOL_BRIGHT_CATEGORICAL = ['#EE6677', '#228833', '#4477AA', '#CCBB44', '#66CCEE', '#AA3377', '#BBBBBB']
HIGH_CONTRAST_CATEGORICAL = ['#FFFFFF', '#FAC200', '#16BBEA', '#00A35C', '#F32B1A', '#9B59B6', '#34495E', '#BDC3C7']

ACCESSIBLE_SEQUENTIAL_PLOTLY_SCALES = ["Viridis", "Cividis", "Plasma", "Blues", "Greens", "Oranges", "PuBu"]
HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES = ["Inferno", "Magma", "OrRd", "YlGnBu", "Greys", "Blackbody"]
...
COLOR_PAPER_BG = "rgba(18, 24, 40, 0)" 
COLOR_PLOT_BG = "rgba(18, 24, 40, 0)"  
...
def plot_key_metrics_summary(compliance: float, proximity: float, wellbeing: float, downtime: float,
    ...
    for title, raw_value, raw_target, bar_color_val, suffix, lower_is_better in metrics_config:
        ...
        axis_max_val = 100.0 if suffix == "%" else max(value * 1.25, target * 1.5, 10.0)
        if abs(value) > axis_max_val * 0.95 : axis_max_val = abs(value) * 1.25 # Ensure value is visible
        if abs(target) > axis_max_val * 0.95 : axis_max_val = abs(target) * 1.25 # Ensure target mark is visible
        ...
        if lower_is_better:
            ...
            if target == 0.0: s1_upper = EPSILON * 2; s2_upper = axis_max_val * 0.25 if axis_max_val > EPSILON else EPSILON * 5
        ...
        for step_conf in steps_config:
            s_l, s_u = step_conf['range']
            s_l = max(current_lower, float(s_l) if pd.notna(s_l) else current_lower)
            s_u = min(axis_max_val, float(s_u) if pd.notna(s_u) else axis_max_val)
...
def plot_collaboration_proximity_index(data, disruption_points=None, forecast_data=None, high_contrast=False):
    # This function plots the 'collaboration_metric', title reflects this.
...
def plot_operational_efficiency(efficiency_df, selected_metrics, disruption_points=None, high_contrast=False):
    ...
    fig = go.Figure()
    metric_styles = {
        'uptime': {'color_idx': 0, 'dash': 'solid', 'width': 1.5},
        'throughput': {'color_idx': 1, 'dash': 'solid', 'width': 1.5},
        'quality': {'color_idx': 2, 'dash': 'solid', 'width': 1.5},
        'oee': {'color_idx': 3, 'dash': 'solid', 'width': 2.5}
    }
        ...
        for dp_step in disruption_points:
             ...
             if actual_x_val in efficiency_df.index or (isinstance(dp_step, int) and 0 <= dp_step < len(efficiency_df)):
                fig.add_vline(x=actual_x_val, line=dict(color=COLOR_WARNING_AMBER, width=1.5, dash="longdash"),
                              annotation_text="▼", annotation_position="top",
                              annotation=dict(font_size=12, font_color=COLOR_WARNING_AMBER, showarrow=False, yshift=10))
    _apply_common_layout_settings(fig, "OEE & Components", high_contrast, # Removed "Overall Equipment Effectiveness" for brevity
...
def plot_worker_distribution(team_positions_df, facility_size, facility_config, use_3d=False, selected_step=0, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    mpi = facility_config.get('MINUTES_PER_INTERVAL', 2) # Get from facility_config if passed by main
    ...
    if df_step.empty:
        return _get_no_data_figure(f"{plot_title} - No data for step", high_contrast)

    facility_width, facility_height = facility_size
    cat_palette = HIGH_CONTRAST_CATEGORICAL if high_contrast else TOL_BRIGHT_CATEGORICAL
    
    if 'zone' not in df_step.columns: df_step['zone'] = 'Unknown'
    else: df_step['zone'] = df_step['zone'].fillna('Unknown')
    ...
    work_area_outline_color = TOL_BRIGHT_CATEGORICAL[4 % len(TOL_BRIGHT_CATEGORICAL)] # A consistent outline color
         ...
         for area_name, area_details in work_areas_cfg.items():
            if isinstance(area_details, dict) and 'coords' in area_details:
                ...
                if isinstance(coords_list, list) and len(coords_list) == 2 and \
                   isinstance(coords_list[0], tuple) and isinstance(coords_list[1], tuple): # Ensure two points
                    (x0,y0), (x1,y1) = coords_list[0], coords_list[1]
                    if all(isinstance(c, (int,float)) for c_tuple in coords_list for c in c_tuple): # Check all coords are numeric
                        shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1),
                                                    ...
                                                    fillcolor="rgba(128,128,128,0.05)", layer="below")) # Subtle fill
...
def plot_worker_density_heatmap(team_positions_df, facility_size, facility_config, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    facility_width, facility_height = facility_size
    heatmap_colorscale = HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES[0] if high_contrast else ACCESSIBLE_SEQUENTIAL_PLOTLY_SCALES[0] # E.g. Inferno or Viridis

    fig = go.Figure(go.Histogram2dContour(
        x=team_positions_df['x'], y=team_positions_df['y'],
        colorscale=heatmap_colorscale, reversescale=False, # Most density scales are light to dark
        showscale=True, line=dict(width=0), # No lines between contour levels for smoother look
        contours=dict(coloring='heatmap', showlabels=False),
        xbins=dict(start=-facility_width*0.05, end=facility_width*1.05, size=facility_width/25), # Bins slightly outside facility for edge cases
        ybins=dict(start=-facility_height*0.05, end=facility_height*1.05, size=facility_height/20),
    ...
    ee_point_color = COLOR_INFO_BLUE
    work_area_outline_color = TOL_BRIGHT_CATEGORICAL[5 % len(TOL_BRIGHT_CATEGORICAL)] # Different from distribution outlines
    ...
    fig.update_xaxes(constrain="domain"); fig.update_yaxes(constrain="domain", scaleanchor="x", scaleratio=1) # Enforce aspect ratio
    return fig
...
def plot_worker_wellbeing(scores, triggers, disruption_points=None, high_contrast=False):
    ...
    for trigger_type, points_data in triggers_data.items():
        ...
        if relative_steps_for_type:
            ...
            valid_y_values = [scores[p] for p in relative_steps_for_type if pd.notna(scores[p])] # Ensure y is not NaN
            valid_x_points = [p for p, y_val in zip(relative_steps_for_type, [scores[pt] for pt in relative_steps_for_type]) if pd.notna(y_val)] # X for non-NaN Y

            if valid_x_points and valid_y_values and len(valid_x_points) == len(valid_y_values):
...
def plot_downtime_trend(downtime_per_step_data, interval_threshold_minutes, disruption_points=None, high_contrast=False):
    if not isinstance(downtime_per_step_data, (list, np.ndarray, pd.Series)) or not any(d > EPSILON for d in downtime_per_step_data): # Check if any actual downtime
    ...
    max_y_val = max(max(downtime_per_step_data) * 1.15 if downtime_per_step_data else 10.0, interval_threshold_minutes * 1.5, 10.0)
...
def plot_downtime_causes_pie(downtime_events_list_for_pie, high_contrast=False):
    ...
    for event in downtime_events_list_for_pie:
        ...
        if cause == "None" or cause == "Unknown" or not cause : continue
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        textfont=dict(color=text_color_pie, size=11), insidetextorientation='radial', # Radial for better fit
    ...
    fig.update_layout(legend_title_text="Downtime Causes" if len(labels) <=7 else None,
                      uniformtext_minsize=9, uniformtext_mode='hide') # Improve text fitting
    return fig
