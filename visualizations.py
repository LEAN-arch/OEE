...
def plot_worker_wellbeing(scores, triggers):
    ...
    try:
        ...
        for t in triggers['threshold']:
            ...
            y_offset += 5  # Increment offset to avoid overlap
        ...
        fig.update_layout(
            ...
            hovermode='x unified',
            plot_bgcolor='#1A252F', paper_bgcolor='#1A252F',
            annotations=annotations[:5],  # Limit to 5 annotations to prevent clutter
            legend=dict(orientation='h', yanchor='top', y=1.1, xanchor='right', x=1),
            xaxis=dict(tickangle=45, nticks=len(minutes)//10 + 1)
        )
        return fig
...
def plot_psychological_safety(scores):
    ...
    try:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=minutes, y=scores, mode='lines+markers', name='Safety Score',
            line=dict(color='#EC4899', width=3),
            hovertemplate='Time: %{x} min<br>Score: %{y:.1f}%'
        ))
        ...
        for i, score in enumerate(scores):
            if score < threshold:
                annotations.append(dict(
                    x=minutes[i], y=score + y_offset, text='Low Safety',
                    showarrow=True, arrowhead=1, ax=20, ay=-30, font=dict(color='#EF4444')
                ))
                y_offset += 5
        ...
        fig.update_layout(
            ...
            hovermode='x unified',
            plot_bgcolor='#1A252F', paper_bgcolor='#1A252F',
            annotations=annotations[:5],  # Limit to 5 annotations
            legend=dict(orientation='h', yanchor='top', y=1.1, xanchor='right', x=1),
            xaxis=dict(tickangle=45, nticks=len(minutes)//10 + 1)
        )
        return fig
