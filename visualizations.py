...
# from plotly.subplots import make_subplots # This import was unused in last version, can be removed if still so.
import numpy as np
import pandas as pd
...
PLOTLY_TEMPLATE = "plotly_dark" # Base template for dark theme

# Standard Theme Colors (for dark background, with good distinguishability)
PRIMARY_COLOR_STD = "#7F7FD5"  # Brighter Purple/Blue for lines/markers
SECONDARY_COLOR_STD = "#22D3EE" # Cyan
ALERT_COLOR_STD = "#FFD700"    # Gold/Brighter Yellow for alerts, thresholds
CRITICAL_COLOR_STD = "#FF6B6B" # Brighter Red for critical issues
HIGHLIGHT_COLOR_STD = "#A78BFA" # Violet for forecasts or highlights
NEUTRAL_COLOR_STD = "#6B7280"  # Medium Gray for gridlines, less important text on dark
POSITIVE_COLOR_STD = "#34D399" # Green

# High Contrast Theme Colors (Typically for light background, but can be adapted)
# These aim for maximum distinguishability regardless of theme.
PRIMARY_COLOR_HC = "#000000"   # Black (if on light bg) or White (if on dark hc bg)
SECONDARY_COLOR_HC = "#E69F00" # Orange (Paul Tol's palette)
ALERT_COLOR_HC = "#D55E00"    # Vermillion (Paul Tol)
CRITICAL_COLOR_HC = "#CC0000" # Darker, distinct Red
HIGHLIGHT_COLOR_HC = "#0072B2" # Blue (Paul Tol)
NEUTRAL_COLOR_HC = "#555555"  # Dark Gray for grids on light, or lighter gray on dark HC
POSITIVE_COLOR_HC = "#009E73" # Bluish Green (Paul Tol)


def _get_colors(high_contrast=False):
    if high_contrast: # For a high-contrast dark theme, we need bright distinguishable colors
        return "#FFFFFF", "#FFFF00", "#FFA500", "#FF4136", "#00FFFF", "#888888", "#00FF00" # White, Yellow, Orange, Red, Cyan, LightGray, Green
...
def _apply_common_layout_settings(fig, title_text, high_contrast=False, yaxis_title=None, xaxis_title="Time Step (Interval)", yaxis_range=None, show_legend=True):
    ...
    font_color = "#EAEAEA" if not high_contrast else "#FFFFFF"
    plot_bgcolor = "#1F2937" if not high_contrast else "#111111" # Slightly darker for HC plots
    grid_color = "#374151" if not high_contrast else "#555555" # Grid color adjusted for theme
    zeroline_color = "#4A5568" if not high_contrast else "#666666"
    line_color = "#4A5568" if not high_contrast else "#777777"


    fig.update_layout(
        template=PLOTLY_TEMPLATE, # Start with plotly_dark
        ...
        legend=dict(
            ...
            bgcolor="rgba(31, 41, 55, 0.7)" if not high_contrast else "rgba(17,17,17,0.7)", 
            bordercolor=n_c, borderwidth=0.5, font_size=10,
            traceorder="normal"
        ) if show_legend else None,
        margin=dict(l=60, r=40, t=70, b=60),
        xaxis=dict(
            title=xaxis_title, gridcolor=grid_color, zerolinecolor=zeroline_color, 
            showline=True, linewidth=1, linecolor=line_color,
            rangemode='tozero' if xaxis_title and "Time" in xaxis_title else 'normal',
            titlefont=dict(size=13), tickfont=dict(size=11)
        ),
        yaxis=dict(
            title=yaxis_title, gridcolor=grid_color, zerolinecolor=zeroline_color, 
            showline=True, linewidth=1, linecolor=line_color,
            range=yaxis_range,
            titlefont=dict(size=13), tickfont=dict(size=11)
        ),
    ...
    if high_contrast: # Further specific HC adjustments
        fig.update_xaxes(showgrid=True, gridwidth=0.5, gridcolor=NEUTRAL_COLOR_HC)
        fig.update_yaxes(showgrid=True, gridwidth=0.5, gridcolor=NEUTRAL_COLOR_HC)
...
def plot_key_metrics_summary(compliance, proximity, wellbeing, downtime, high_contrast=False):
    ...
    font_color_gauge = "#EAEAEA" if not high_contrast else "#FFFFFF"
    gauge_bgcolor = "#2a3447" if not high_contrast else "#222222"
    ...
    for title, value, target, bar_color, pos_delta_c, neg_delta_c, suffix, lower_is_better in metrics_config:
        ...
        fig.add_trace(go.Indicator(
            ...
            gauge={
                'axis': {'range': [0, 100 if suffix == "%" else max(target * 1.5, value * 1.2 if value else target * 1.5, 10)], 'tickwidth': 1, 'tickcolor': n_c}, # Ensure value is not None for calc
                ...
                'steps': [
                    ...
                    {'range': [target * (0.8 if not lower_is_better else 1.2), target * (1.0 if not lower_is_better else 9999)], 'color': a_c}, 
                ],
...
def plot_task_compliance_score(data, disruption_points, forecast_data=None, z_scores=None, high_contrast=False):
    if not data: return go.Figure().update_layout(title_text="Task Compliance (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    ...
    for dp_step in disruption_points: 
        if 0 <= dp_step < len(data): fig.add_vline(x=dp_step, line=dict(color=a_c, width=1.5, dash="longdash"), annotation_text="D", annotation_position="top left", annotation=dict(font_size=10, bgcolor="rgba(40,40,40,0.7)", borderpad=2, textangle=0, font_color = p_c if high_contrast else "#EAEAEA")) 
...
def plot_collaboration_proximity_index(data, disruption_points, forecast_data=None, high_contrast=False):
    if not data: return go.Figure().update_layout(title_text="Collaboration Index (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure(); x_vals = list(range(len(data)))
    ...
    if forecast_data and len(forecast_data)==len(data): fig.add_trace(go.Scatter(x=x_vals, y=forecast_data, mode='lines', name='Forecast', line=dict(color=h_c, dash='dashdot', width=2), hovertemplate='Forecast: %{y:.1f}%<extra></extra>'))
    for dp_step in disruption_points:
        if 0 <= dp_step < len(data): fig.add_vline(x=dp_step, line=dict(color=a_c, width=1.5, dash="longdash"), annotation_text="D", annotation_position="bottom right", annotation=dict(font_size=10, bgcolor="rgba(40,40,40,0.7)", borderpad=2, textangle=0, font_color=p_c if high_contrast else "#EAEAEA"))
...
def plot_operational_recovery(recovery_data, productivity_loss_data, high_contrast=False):
    if not recovery_data: return go.Figure().update_layout(title_text="Operational Recovery (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
...
def plot_operational_efficiency(efficiency_df, selected_metrics, high_contrast=False):
    if efficiency_df.empty: return go.Figure().update_layout(title_text="Operational Efficiency (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure()
    colors = {'uptime': p_c, 'throughput': s_c, 'quality': h_c, 'oee': pos_c}; line_styles = {'uptime': 'solid', 'throughput': 'solid', 'quality': 'solid', 'oee': 'solid'}; line_widths = {'uptime': 1.5, 'throughput': 1.5, 'quality': 1.5, 'oee': 2.5} # OEE slightly thicker
...
def plot_worker_distribution(team_positions_df, facility_size, config, use_3d=False, selected_step=0, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    if df_step.empty: return go.Figure().update_layout(title_text=f"Worker Distribution (No Data for Step {selected_step})", template=PLOTLY_TEMPLATE, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data for this step.", showarrow=False)
    facility_width, facility_height = facility_size; zone_names = df_step['zone'].unique(); color_map_base = px.colors.qualitative.Vivid if not high_contrast else px.colors.qualitative.Safe; zone_color_map = {zone: color_map_base[i % len(color_map_base)] for i, zone in enumerate(zone_names)}
    ...
    if show_entry_exit and 'ENTRY_EXIT_POINTS' in config:
        for point in config['ENTRY_EXIT_POINTS']: shapes.append(go.layout.Shape(type="circle", x0=point['coords'][0]-1.5, y0=point['coords'][1]-1.5, x1=point['coords'][0]+1.5, y1=point['coords'][1]+1.5, fillcolor=a_c, line_color=p_c if high_contrast else "black", line_width=1, opacity=0.9, layer="above")); annotations.append(dict(x=point['coords'][0], y=point['coords'][1]+4, text=point['name'][:2].upper(), showarrow=False, font=dict(size=9, color=p_c if high_contrast else "#FFFFFF" if not high_contrast else "#000000"), borderpad=2, bgcolor="rgba(0,0,0,0.3)" if not high_contrast else "rgba(200,200,200,0.3)"))
    if show_prod_lines and 'WORK_AREAS' in config:
         for area_name, area_details in config['WORK_AREAS'].items():
            if 'coords' in area_details: (x0,y0), (x1,y1) = area_details['coords']; shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1), line=dict(color=s_c, dash="dashdot", width=1.5), fillcolor="rgba(34,211,238,0.05)" if not high_contrast else "rgba(0,255,255,0.05)", layer="below")); annotations.append(dict(x=(x0+x1)/2, y=min(y0,y1)-4, text=area_name, showarrow=False, font=dict(size=9, color=n_c), opacity=0.9, yanchor="top")) 
...
def plot_worker_density_heatmap(team_positions_df, facility_size, config, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    heatmap_colorscale = 'Viridis' if not high_contrast else [[0, 'rgb(50,50,50)'], [1, 'rgb(220,220,220)']] # Custom HC grayscale
    fig = go.Figure(go.Histogram2dContour(x=team_positions_df['x'], y=team_positions_df['y'], colorscale=heatmap_colorscale, reversescale=False, showscale=True, line=dict(width=0.3, color=n_c if high_contrast else "#333"), contours=dict(coloring='heatmap', showlabels=False), xbins=dict(start=0, end=facility_width, size=facility_width/20), ybins=dict(start=0, end=facility_height, size=facility_height/20), colorbar=dict(title='Density', thickness=15, len=0.75, y=0.5, tickfont_size=10, x=1.05,bgcolor="rgba(0,0,0,0.1)", bordercolor=n_c)))
    ...
    if show_entry_exit and 'ENTRY_EXIT_POINTS' in config:
        for point in config['ENTRY_EXIT_POINTS']: shapes.append(go.layout.Shape(type="circle", x0=point['coords'][0]-1, y0=point['coords'][1]-1, x1=point['coords'][0]+1, y1=point['coords'][1]+1, fillcolor=a_c, line_color=p_c if high_contrast else "white", opacity=0.6, layer="above"))
    if show_prod_lines and 'WORK_AREAS' in config:
         for area_name, area_details in config['WORK_AREAS'].items():
            if 'coords' in area_details: (x0,y0), (x1,y1) = area_details['coords']; shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1), line=dict(color=s_c, dash="dot", width=1), fillcolor="rgba(0,0,0,0)", layer="above"))
...
def plot_worker_wellbeing(scores, triggers, high_contrast=False):
    if not scores: return go.Figure().update_layout(title_text="Well-being (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    ...
    trigger_colors = {'threshold': cr_c, 'trend': a_c, 'disruption': h_c, 'work_area_general': s_c}; trigger_symbols = {'threshold': 'x-thin', 'trend': 'triangle-down-open', 'disruption': 'star-open', 'work_area_general': 'diamond-open'}
...
def plot_psychological_safety(data, high_contrast=False):
    if not data: return go.Figure().update_layout(title_text="Psych. Safety (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure(); x_vals = list(range(len(data)))
...
def plot_downtime_trend(downtime_events_list, interval_threshold, high_contrast=False):
    if not downtime_events_list: return go.Figure().update_layout(title_text="Downtime Trend (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure(); 
...
def plot_team_cohesion(data, high_contrast=False):
    if not data: return go.Figure().update_layout(title_text="Team Cohesion (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure(); x_vals = list(range(len(data)))
...
def plot_perceived_workload(data, high_workload_threshold, very_high_workload_threshold, high_contrast=False):
    if not data: return go.Figure().update_layout(title_text="Perceived Workload (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No data available.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast); fig = go.Figure(); x_vals = list(range(len(data)))
    line_color_workload = n_c if not high_contrast else p_c # Use neutral for line, markers colored by threshold
    marker_colors = [cr_c if val >= very_high_workload_threshold else a_c if val >= high_workload_threshold else pos_c for val in data] 
    fig.add_trace(go.Scatter(x=x_vals, y=data, mode='lines+markers', name='Perceived Workload', line=dict(color=line_color_workload, width=2), marker=dict(size=6, color=marker_colors), hovertemplate='Workload: %{y:.1f}/10<extra></extra>'))
    fig.add_hline(y=high_workload_threshold, line=dict(color=a_c, width=1.5, dash="dash"), annotation_text=f"High ({high_workload_threshold})", annotation_position="bottom right", annotation_font=dict(color=a_c, size=10))
...
def plot_downtime_causes_pie(downtime_events_list, high_contrast=False):
    if not downtime_events_list: return go.Figure().update_layout(title_text="Downtime Causes (No Data)", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#1F2937" if not high_contrast else "#101010").add_annotation(text="No downtime events to analyze.", showarrow=False)
    p_c, s_c, a_c, cr_c, h_c, n_c, pos_c = _get_colors(high_contrast)
    ...
    color_sequence = px.colors.qualitative.Pastel if not high_contrast else px.colors.qualitative.Bold 
    pie_colors = [color_sequence[i % len(color_sequence)] for i in range(num_causes)]
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=.4, pull=[0.02]*num_causes, marker_colors=pie_colors, textinfo='label+percent', insidetextorientation='auto', hovertemplate="<b>Cause:</b> %{label}<br><b>Duration:</b> %{value:.1f} min<br><b>Share:</b> %{percent}<extra></extra>", sort=True, direction='descending')])
    _apply_common_layout_settings(fig, f"Downtime Distribution by Cause (Total: {total_downtime_duration_for_pie:.0f} min)", high_contrast, show_legend=False if num_causes > 6 else True) # Hide legend if too many causes
    fig.update_traces(textfont_size=10, marker=dict(line=dict(color=p_c if high_contrast else '#000000', width=0.5))) # Add border to slices
    return fig
