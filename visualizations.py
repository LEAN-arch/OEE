...
# --- Accessible Color Definitions for LIGHT GRAY THEME ---
COLOR_PAGE_BACKGROUND_LIGHT = "#F0F2F6" 
COLOR_PAPER_BG_LIGHT = COLOR_PAGE_BACKGROUND_LIGHT 
COLOR_PLOT_BG_LIGHT = COLOR_PAGE_BACKGROUND_LIGHT  

COLOR_PRIMARY_TEXT_DARK = "#262730" 
COLOR_SECONDARY_TEXT_DARK = "#5E6474" 
COLOR_WHITE_ON_DARK_BG = "#FFFFFF" 

COLOR_GRID_LIGHT_THEME = "#D0D0D0" 
COLOR_AXIS_LINE_LIGHT_THEME = "#B0B0B0" 

COLOR_CRITICAL_RED_LIGHT_THEME = "#D62728" 
...
HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES_LIGHT = ["OrRd", "PuBuGn", "YlGnBu", "Inferno", "Magma", "Blackbody", "Hot"]
...
def plot_key_metrics_summary(compliance: float, proximity: float, wellbeing: float, downtime: float,
                             ...
                             accent_color: str = COLOR_ACCENT_PURPLE_LIGHT_THEME):
    figs = []
...
def plot_collaboration_proximity_index(data, disruption_points=None, forecast_data=None, high_contrast=False):
    ...
    fig.add_trace(go.Scatter(x=x_vals, y=data, mode='lines+markers', name='Collab. Metric',
                             ...
                             marker=dict(size=6, symbol="diamond", line=dict(width=1, color=COLOR_PAPER_BG_LIGHT if high_contrast else cat_palette[1 % len(cat_palette)])),
...
def plot_worker_distribution(team_positions_df, facility_size, facility_config, use_3d=False, selected_step=0, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    if use_3d:
        ...
        fig.update_scenes(aspectmode='data', xaxis_showgrid=False, yaxis_showgrid=False, zaxis_showgrid=False, xaxis_backgroundcolor=COLOR_PLOT_BG_LIGHT, yaxis_backgroundcolor=COLOR_PLOT_BG_LIGHT, zaxis_backgroundcolor=COLOR_PLOT_BG_LIGHT)
    else:
        ...
        grid_c = COLOR_GRID_LIGHT_THEME; fig.update_layout(xaxis_showgrid=True, yaxis_showgrid=True, xaxis_gridcolor=grid_c, yaxis_gridcolor=grid_c, xaxis_zeroline=True, yaxis_zeroline=True, xaxis_zerolinecolor=grid_c, yaxis_zerolinecolor=grid_c)

    shapes = [go.layout.Shape(type="rect", x0=0, y0=0, x1=facility_width, y1=facility_height, line=dict(color=COLOR_NEUTRAL_GRAY_LIGHT_THEME, width=1.5), fillcolor="rgba(0,0,0,0)", layer="below")]
    annotations = []; font_c = COLOR_PRIMARY_TEXT_DARK; ee_point_color = COLOR_INFO_BLUE_LIGHT_THEME
            ...
            if isinstance(area_details, dict) and 'coords' in area_details:
                ...
                if isinstance(coords_list, list) and len(coords_list) == 2 and isinstance(coords_list[0], tuple) and len(coords_list[0])==2 and isinstance(coords_list[1], tuple) and len(coords_list[1])==2:
                    ...
                    if all(isinstance(c, (int,float)) for c_tuple in coords_list for c in c_tuple):
                        shapes.append(go.layout.Shape(type="rect", x0=min(x0,x1), y0=min(y0,y1), x1=max(x0,x1), y1=max(y0,y1), line=dict(color=work_area_outline_color, dash="dashdot", width=1.5), fillcolor="rgba(200,200,200,0.05)", layer="below"))
                        annotations.append(dict(x=(x0+x1)/2, y=min(y0,y1)-4, text=area_name, showarrow=False, font=dict(size=9, color=COLOR_SECONDARY_TEXT_DARK), opacity=0.9, yanchor="top"))
...
def plot_worker_density_heatmap(team_positions_df, facility_size, facility_config, show_entry_exit=True, show_prod_lines=True, high_contrast=False):
    ...
    if show_entry_exit and entry_exit_cfg:
        for point in entry_exit_cfg:
            if isinstance(point, dict) and 'coords' in point and isinstance(point['coords'], tuple) and len(point['coords']) == 2:
                px_ee, py_ee = point['coords']; shapes.append(go.layout.Shape(type="circle", x0=px_ee-1, y0=py_ee-1, x1=px_ee+1, y1=py_ee+1, fillcolor=ee_point_color, line_color=COLOR_PRIMARY_TEXT_DARK, opacity=0.6, layer="above"))
    if show_prod_lines and work_areas_cfg:
...
def plot_worker_wellbeing(scores, triggers, disruption_points=None, high_contrast=False):
    ...
    if avg_wellbeing is not None: fig.add_hline(y=avg_wellbeing, line=dict(color=COLOR_NEUTRAL_GRAY_LIGHT_THEME, width=1, dash="dot"), annotation_text=f"Avg: {avg_wellbeing:.1f}%", annotation_position="bottom left", annotation_font_size=10, annotation_font_color=COLOR_NEUTRAL_GRAY_LIGHT_THEME)
    if disruption_points:
        for dp_step in disruption_points:
            if 0 <= dp_step < len(scores): fig.add_vline(x=dp_step, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="longdash"), annotation_text="▼", annotation_position="top", annotation=dict(font_size=12, font_color=COLOR_WARNING_AMBER_LIGHT_THEME, showarrow=False, yshift=10))
    trigger_styles = {'threshold': {'color': COLOR_CRITICAL_RED_LIGHT_THEME, 'symbol': 'x-dot', 'name': 'Threshold Alert', 'size': 10}, 'trend': {'color': COLOR_WARNING_AMBER_LIGHT_THEME, 'symbol': 'triangle-down-dot', 'name': 'Trend Alert', 'size': 10}, 'disruption': {'color': COLOR_INFO_BLUE_LIGHT_THEME, 'symbol': 'star-dot', 'name': 'Disruption Link', 'size': 10}, 'work_area': {'color': cat_palette[2 % len(cat_palette)], 'symbol': 'diamond-dot', 'name': 'Work Area Alert', 'size': 9}}
    ...
    for trigger_type, points_data in triggers_data.items():
        ...
        if relative_steps_for_type:
            ...
            if valid_x_points and valid_y_values: fig.add_trace(go.Scatter(x=valid_x_points, y=valid_y_values, mode='markers', name=style['name'], marker=dict(color=style['color'], size=style['size'], symbol=style['symbol'], line=dict(width=1.5, color=COLOR_PLOT_BG_LIGHT if high_contrast else style['color'])), hovertemplate=f'{style["name"]}: %{{y:.1f}}% at Step %{{x}}<extra></extra>' ))
...
def plot_psychological_safety(data, disruption_points=None, high_contrast=False):
    ...
    if avg_safety is not None: fig.add_hline(y=avg_safety, line=dict(color=COLOR_NEUTRAL_GRAY_LIGHT_THEME, width=1, dash="dot"), annotation_text=f"Avg: {avg_safety:.1f}%", annotation_position="bottom left", annotation_font_size=10, annotation_font_color=COLOR_NEUTRAL_GRAY_LIGHT_THEME)
    if disruption_points:
        for dp_step in disruption_points:
            if 0 <= dp_step < len(data): fig.add_vline(x=dp_step, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="longdash"), annotation_text="▼", annotation_position="top", annotation=dict(font_size=12, font_color=COLOR_WARNING_AMBER_LIGHT_THEME, showarrow=False, yshift=10))
...
def plot_downtime_trend(downtime_per_step_data, interval_threshold_minutes, disruption_points=None, high_contrast=False):
    ...
    fig.add_hline(y=interval_threshold_minutes, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="dash"), annotation_text=f"Alert: {interval_threshold_minutes} min", annotation_position="top right", annotation_font=dict(size=10, color=COLOR_WARNING_AMBER_LIGHT_THEME), annotation_bgcolor="rgba(255,255,255,0.6)")
...
def plot_team_cohesion(data, disruption_points=None, high_contrast=False):
    ...
    if avg_cohesion is not None: fig.add_hline(y=avg_cohesion, line=dict(color=COLOR_NEUTRAL_GRAY_LIGHT_THEME, width=1, dash="dot"), annotation_text=f"Avg: {avg_cohesion:.1f}%", annotation_position="top left", annotation_font_size=10, annotation_font_color=COLOR_NEUTRAL_GRAY_LIGHT_THEME)
    if disruption_points:
        for dp_step in disruption_points:
            if 0 <= dp_step < len(data): fig.add_vline(x=dp_step, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="longdash"), annotation_text="▼", annotation_position="top", annotation=dict(font_size=12, font_color=COLOR_WARNING_AMBER_LIGHT_THEME, showarrow=False, yshift=10))
...
def plot_perceived_workload(data, high_workload_threshold, very_high_workload_threshold, disruption_points=None, high_contrast=False):
    ...
    marker_colors = [COLOR_CRITICAL_RED_LIGHT_THEME if val >= very_high_workload_threshold else COLOR_WARNING_AMBER_LIGHT_THEME if val >= high_workload_threshold else COLOR_POSITIVE_GREEN_LIGHT_THEME for val in data]
    fig.add_trace(go.Scatter(x=x_vals, y=data, mode='lines+markers', name='Perceived Workload', line=dict(color=line_color_main, width=2), marker=dict(size=6, color=marker_colors, line=dict(width=1, color=COLOR_PAPER_BG_LIGHT if high_contrast else line_color_main)), hovertemplate='Workload: %{y:.1f}/10<extra></extra>'))
    fig.add_hline(y=high_workload_threshold, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="dash"), annotation_text=f"High ({high_workload_threshold})", annotation_position="bottom right", annotation_font=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, size=10))
    fig.add_hline(y=very_high_workload_threshold, line=dict(color=COLOR_CRITICAL_RED_LIGHT_THEME, width=1.5, dash="dash"), annotation_text=f"V.High ({very_high_workload_threshold})", annotation_position="top right", annotation_font=dict(color=COLOR_CRITICAL_RED_LIGHT_THEME, size=10))
    if disruption_points:
        for dp_step in disruption_points:
            if 0 <= dp_step < len(data): fig.add_vline(x=dp_step, line=dict(color=COLOR_WARNING_AMBER_LIGHT_THEME, width=1.5, dash="longdash"), annotation_text="▼", annotation_position="top", annotation=dict(font_size=12, font_color=COLOR_WARNING_AMBER_LIGHT_THEME, showarrow=False, yshift=10))
...
def plot_downtime_causes_pie(downtime_events_list_for_pie, high_contrast=False):
    ...
    slice_border_color = COLOR_GRID_LIGHT_THEME
    text_color_pie = COLOR_PRIMARY_TEXT_DARK
