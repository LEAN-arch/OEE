# config.py
import numpy as np
import logging # Added for validate_config logging

logger = logging.getLogger(__name__)

DEFAULT_CONFIG = {
    'TEAM_SIZE': 30, # Reduced for potentially faster initial simulation
    ...
    'FACILITY_SIZE': (100, 80),  # Facility dimensions (width, height) in meters
    ...
    'ENTRY_EXIT_POINTS': [
        {'name': 'Main Entrance', 'coords': (0, 40), 'type': 'entry_exit'},
        {'name': 'Loading Dock', 'coords': (100, 60), 'type': 'exit_only'}
    ],
...
# Dynamically set SHIFT_DURATION_INTERVALS
...
def validate_config(config):
    ...
    if not (isinstance(config['FACILITY_SIZE'], tuple) and len(config['FACILITY_SIZE']) == 2 and all(isinstance(x, (int, float)) for x in config['FACILITY_SIZE'])):
        raise ValueError("FACILITY_SIZE must be a tuple of two numbers (width, height).")

    if not isinstance(config['WORK_AREAS'], dict) or not config['WORK_AREAS']:
        raise ValueError("WORK_AREAS must be a non-empty dictionary.")
    
    for area_name, area_details in config['WORK_AREAS'].items():
        if not isinstance(area_details, dict) or 'coords' not in area_details or 'workers' not in area_details:
            raise ValueError(f"Work area '{area_name}' is missing required keys 'coords' or 'workers' or is not a dictionary.")

    total_workers_in_zones = sum(zone.get('workers', 0) for zone in config['WORK_AREAS'].values() if isinstance(zone, dict))
    if total_workers_in_zones != config['TEAM_SIZE']:
        logger.warning(
            f"Config Validation: Sum of workers in WORK_AREAS ({total_workers_in_zones}) "
            f"does not precisely match TEAM_SIZE ({config['TEAM_SIZE']}). Simulation logic will attempt to reconcile this."
        )
    
    logger.info("Configuration structure validated. Specific value ranges may be checked during simulation.", extra={'user_action': 'System Check'})
    return True
