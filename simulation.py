...
def simulate_workplace_operations(num_team_members: int, num_steps: int, 
    ...
    event_type_config = _get_config_param(config, 'EVENT_TYPE_CONFIG', {})
    
    # Get shift_duration_minutes from the config passed to the simulation
    # This is set in main.py's run_simulation_logic based on sidebar input
    shift_duration_minutes_sim = float(_get_config_param(config, 'SHIFT_DURATION_MINUTES', 480)) # Ensure float
    ...
    for step in range(num_steps):
        ...
        is_any_disruption_active = False 

        active_events_this_step = []
        for event_details in scheduled_events: # Use the passed 'scheduled_events'
            if not isinstance(event_details, dict): continue 
            ...
            if event_start_min <= current_minute_of_shift < event_start_min + event_duration_min:
                active_events_this_step.append(event_details) 
                ...
                if "Disruption" in event_type: 
                    ...
                    current_event_effects["fatigue_rate_modifier"] = max(current_event_effects["fatigue_rate_modifier"], 1.3) 
                    if current_minute_of_shift == event_start_min and step < num_steps: # Ensure step is within bounds
                    ...
                    productivity_during_meeting = event_params.get("productivity_during_meeting", 0.1)
                    if not affected_zones_meeting or "All" in affected_zones_meeting: 
                        for zone_name in work_areas_config.keys():
                            current_event_effects["specific_zone_effects"].setdefault(zone_name, {})["productivity_multiplier"] = productivity_during_meeting
                    else:
        ...
        for zone_name, zone_details in work_areas_config.items():
            tasks_arriving = zone_details.get('tasks_per_interval', 0) * (1.0 + (0.2*random.random()-0.1) if is_any_disruption_active else 0.0)
        ...
        fatigue_rate_this_step *= current_event_effects["fatigue_rate_modifier"] 
        if num_team_members > 0:
        ...
        wb_now -= current_event_effects["wellbeing_drop_factor"] * 25.0 
        wb_now += current_event_effects["wellbeing_boost_abs"] 
        ...
        if team_initiative != "Increased Autonomy" and not any(event.get("Event Type") == "Increased Autonomy Mode" for event in active_events_this_step): 
        ...
        ps_now += current_event_effects["psych_safety_boost_abs"] 
        ...
        cohesion_now -= (0.15 + (0.7 if is_any_disruption_active else 0.0) + (_perceived_workload_scores[step] / 50.0)) 
        ...
        if is_any_disruption_active and _wellbeing_scores[step] < (_wellbeing_scores[max(0, step - 1)] if step > 0 else _wellbeing_scores[0]) * 0.90:
            if num_team_members > 0:
                affected_worker_indices = [i for i in range(num_team_members) if worker_fatigue[i] > 0.65 or random.random() < current_event_effects.get("compliance_reduction_factor", 0.0) * 0.2] 
        ...
        if is_any_disruption_active: 
            current_uptime -= 30.0 * current_event_effects.get("compliance_reduction_factor", 0.0) 
        ...
        if is_any_disruption_active: quality_now -= 12.0 * current_event_effects.get("compliance_reduction_factor", 0.0) 
        ...
        throughput_disruption_impact = 0.8 * current_event_effects.get("compliance_reduction_factor", 0.0) 
        ...
        if current_event_effects["downtime_prob_modifier"] > 0 and random.random() < current_event_effects["downtime_prob_modifier"]:
            ...
            downtime_std_event = _get_config_param(config, 'DOWNTIME_STD_MINUTES_PER_OCCURRENCE', 4.0) * math.sqrt(current_event_effects["downtime_mean_modifier"])
            ...
            if active_disrupt_types: current_downtime_cause = active_disrupt_types[0] 
        ...
        # Use shift_duration_minutes_sim (defined at the start of the function)
        ...
        if is_any_disruption_active: base_collab -= 25.0 * current_event_effects.get("compliance_reduction_factor", 0.0) 
        ...
        if num_team_members > 0 : 
            for i in range(num_team_members):
                ...
                for active_event in active_events_this_step:
                    ...
                    if event_type_active in ["Scheduled Break", "Short Pause"]:
                        ...
                        if scope == "All" or current_assigned_zone_for_worker_i in affected_zones_event:
                            is_on_scheduled_break = True; break
                ...
                elif is_any_disruption_active and random.random() < 0.3: status_now = 'disrupted' 
