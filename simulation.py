...
def simulate_workplace_operations(num_team_members, num_steps, disruption_event_steps, team_initiative, config):
    ...
    if not worker_zone_names_from_config and num_team_members > 0: # Critical fallback if no zones
    ...
    if num_team_members > 0:
        ...
        else: # Fallback if logic above failed to produce correct length (should not happen with fixes)
            logger.error("Worker assignment logic error, falling back to even distribution over available zones.")
    ...
    zone_task_backlog = {zn: 0 for zn in worker_zone_names_from_config if zn in work_areas_config}
    ...
    for step in range(num_steps):
        ...
        for zone_name, zone_details in work_areas_config.items():
            ...
            # Use previous step's compliance, or an initial estimate if step 0
        ...
        # Normalize workload: backlog relative to capacity (#workers * task capacity) + time pressure
        max_tasks_per_interval_total = sum(zd.get('max_concurrent_tasks', zd.get('tasks_per_interval',0)*1.5) for zd in work_areas_config.values())
        workload_pressure_from_backlog = total_backlog / (max_tasks_per_interval_total + 1e-6)
        _perceived_workload_scores[step] = np.clip(workload_pressure_from_backlog * 10 + (step / num_steps) * 2, 0, 10) # Range 0-10, slight increase over shift
        ...
        wb_now -= (avg_fatigue * 15 + (_perceived_workload_scores[step] - 5) * 0.75) # Increased impact from workload
        ...
        if collab_score_prev < 45 : wb_now -= config.get('ISOLATION_IMPACT_ON_WELLBEING',0.1) * (45 - collab_score_prev) * 0.3 
        ...
        if num_team_members > 0 : worker_fatigue *= 0.65 
        ...
        cohesion_now -= 0.1 + active_disruption_intensity * 0.7 # Disruptions strain cohesion more
        if _psych_safety_scores[step] > 75 : cohesion_now += 0.7 
        if collab_score_prev > 70 : cohesion_now += 0.7 
        if team_initiative == "Team recognition" and step > 0 and step % (num_steps // random.randint(2,4) if num_steps > 3 else 1) == 0: cohesion_now += 4
        _team_cohesion_scores[step] = np.clip(cohesion_now, 15, 100) # Min cohesion slightly higher

        if _wellbeing_scores[step] < config.get('WELLBEING_ALERT_THRESHOLD', 60.0): wellbeing_triggers_dict['threshold'].append(step) # Use percentage value from config
        ...
        if random.random() < config.get('EQUIPMENT_FAILURE_PROB_PER_INTERVAL', 0.005): current_uptime -= random.uniform(10,30) 
        current_uptime -= active_disruption_intensity * 30 
        _uptime_percent[step] = np.clip(current_uptime, 20, 100) # Min uptime can be lower
        ...
        if num_team_members > 0 and worker_assigned_zone and all(z in work_areas_config for z in worker_assigned_zone) : # Ensure all assigned zones exist
        ...
        compliance_now *= (_psych_safety_scores[step]/100 * 0.15 + 0.85) # Psych safety has slightly less impact on compliance here
        compliance_now = max(config.get('MIN_COMPLIANCE_DURING_DISRUPTION',20), compliance_now * (1 - active_disruption_intensity * config.get('DISRUPTION_COMPLIANCE_REDUCTION_FACTOR',0.5)))
        _task_compliance_scores[step] = np.clip(compliance_now, 0, 100)

        _quality_rate_percent[step] = np.clip( (100 - config.get('BASE_QUALITY_DEFECT_RATE',0.02)*100) * (_task_compliance_scores[step]/100)**1.2 , 30, 100) 
        ...
        effective_capacity_factor = (_uptime_percent[step]/100) * (_task_compliance_scores[step]/100) * (1 - avg_fatigue * 0.5) * (1 - _perceived_workload_scores[step]/30) # Workload affects throughput more gradually
        actual_units_produced = max_potential_throughput_facility * effective_capacity_factor * (1 - active_disruption_intensity * 0.8) # Very strong throughput hit from disruption
        ...
        if _uptime_percent[step] < 50 and random.random() < config.get('DOWNTIME_FROM_EQUIPMENT_FAILURE_PROB', 0.7): # If uptime is very low
             current_downtime_this_interval += max(0, np.random.normal(config.get('DOWNTIME_MEAN_MINUTES_PER_OCCURRENCE',8)*0.5, config.get('DOWNTIME_STD_MINUTES_PER_OCCURRENCE',4)*0.3))
        _downtime_minutes_per_interval[step] = np.clip(current_downtime_this_interval, 0, (config.get('SHIFT_DURATION_MINUTES', 480) / (num_steps  + 1e-6) ) * 2 ) 
        ...
        _productivity_loss_percent[step] = np.clip(100 - _operational_recovery_scores[step] + np.random.normal(0,0.5), 0, 100) # Smaller noise for productivity loss

        base_collab = 60 + (_team_cohesion_scores[step]-70)*0.4 - (_perceived_workload_scores[step]-5)*2.5 # Stronger influence of cohesion and workload
        base_collab -= active_disruption_intensity * 20
        _collaboration_scores[step] = np.clip(base_collab + np.random.normal(0,2) ,5, 95) # Reduced noise
        ...
        if num_team_members > 0 : 
            for i in range(num_team_members):
                ...
                status_now = 'working';
                if worker_fatigue[i] > 0.85: status_now = 'exhausted' # More sensitive to exhausted state
