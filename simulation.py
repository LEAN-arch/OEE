...
def simulate_workplace_operations(num_team_members, num_steps, disruption_event_steps, team_initiative, config):
    ...
    if not worker_zone_names_from_config and num_team_members > 0: 
        logger.warning("Simulation: No WORK_AREAS defined. Assigning to 'FallbackZone'.")
    ...
    if num_team_members > 0:
        ...
        if configured_workers_sum == num_team_members :
            for zone_name, zone_details in work_areas_config.items(): worker_assigned_zone.extend([zone_name] * zone_details.get('workers',0))
            ...
            for i in range(num_team_members): worker_assigned_zone.append(worker_zone_names_from_config[i % len(worker_zone_names_from_config)]) 
        if len(worker_assigned_zone) == num_team_members: random.shuffle(worker_assigned_zone)
        elif num_team_members > 0 : 
            logger.error(f"Worker assigned zone length mismatch: {len(worker_assigned_zone)} vs team_size {num_team_members}. Fallback incomplete."); 
            ...
            if len(worker_assigned_zone) != num_team_members: # Final desperate fallback
    ...
    zone_task_backlog = {zn: 0 for zn in work_areas_config.keys()} # Use keys from actual config now

    active_disruption_intensity = 0.0; disruption_linger_factor = config.get('RECOVERY_HALFLIFE_INTERVALS', 10); # Misnomer, using it for linger here.
    disruption_linger_factor = 1 - (1 / (disruption_linger_factor + 1e-6)) # Convert halflife to decay factor
    ...
    for step in range(num_steps):
        ...
        workload_pressure_from_backlog = total_backlog / (max_concurrent_total + 1e-6) # Ratio of backlog to max capacity
        _perceived_workload_scores[step] = np.clip(workload_pressure_from_backlog * 10 + (step / (num_steps + 1e-6)) * 1.5, 0, 10) # Adding slight time pressure increase
        ...
        fatigue_rate_this_step *= (1 + _perceived_workload_scores[step]/12) 
        ...
        wb_now -= (avg_fatigue * 20 + (_perceived_workload_scores[step] - 5) * 1.0) # Stronger impact
        wb_now -= active_disruption_intensity * config.get('DISRUPTION_WELLBEING_DROP', 0.2) * 25 
        leadership_support = config.get('LEADERSHIP_SUPPORT_FACTOR', 0.5) # New config
        wb_now += (leadership_support - 0.5) * 10 # Supportive leadership boosts well-being slightly
        ...
        ps_now += (leadership_support - 0.5) * 15 # Leadership support crucial for psych safety
        ps_now += (config.get('COMMUNICATION_EFFECTIVENESS_FACTOR', 0.5) - 0.5) * 10 # Good comms helps
        ...
        _psych_safety_scores[step] = np.clip(ps_now, 10, 100)

        cohesion_now = _team_cohesion_scores[max(0,step-1)] if step > 0 else config.get('TEAM_COHESION_BASELINE', 0.7) * 100
        cohesion_now -= (0.15 + active_disruption_intensity * 0.6 + (_perceived_workload_scores[step]/50) )# Higher workload strains cohesion
        if _psych_safety_scores[step] > 70 : cohesion_now += 0.8 
        if collab_score_prev > 65 : cohesion_now += 0.6 
        ...
        if random.random() < config.get('EQUIPMENT_FAILURE_PROB_PER_INTERVAL', 0.005): current_uptime -= random.uniform(15,35) 
        ...
        if num_team_members > 0 and worker_assigned_zone and all(z in work_areas_config for z in worker_assigned_zone) : 
        ...
        compliance_now *= (_psych_safety_scores[step]/100 * 0.1 + 0.9) # Reduced psych safety impact a bit
        compliance_now *= (config.get('COMMUNICATION_EFFECTIVENESS_FACTOR',0.7) * 0.3 + 0.7) # Communication up to 30% impact
        compliance_now = max(config.get('MIN_COMPLIANCE_DURING_DISRUPTION',20), compliance_now * (1 - active_disruption_intensity * config.get('DISRUPTION_COMPLIANCE_REDUCTION_FACTOR',0.6))) # Higher reduction factor
        _task_compliance_scores[step] = np.clip(compliance_now, 0, 100)

        _quality_rate_percent[step] = np.clip( (100 - config.get('BASE_QUALITY_DEFECT_RATE',0.02)*100) * (_task_compliance_scores[step]/100)**1.1 , 30, 100) # Slightly less steep quality falloff
        _quality_rate_percent[step] -= active_disruption_intensity * 15
        ...
        if _uptime_percent[step] < 50 and equipment_failed_this_step: # If explicit equipment failure flag triggered uptime drop
             if random.random() < config.get('DOWNTIME_FROM_EQUIPMENT_FAILURE_PROB', 0.7):
                duration_equip_fail = config.get('EQUIPMENT_DOWNTIME_IF_FAIL_INTERVALS',3) * 2 
                current_downtime_duration = max(current_downtime_duration, duration_equip_fail) # Take the max if multiple causes in one step
                current_downtime_cause = "Equipment Failure" if current_downtime_cause == "None" else f"{current_downtime_cause}, Equip.Fail"
        if (_task_compliance_scores[step] < 50 or avg_fatigue > 0.9) and random.random() < 0.03: # Higher chance of human error with very low compliance/high fatigue
        ...
        base_collab = 60 + (_team_cohesion_scores[step]-70)*0.5 - (_perceived_workload_scores[step]-5)*2.0 
        base_collab -= active_disruption_intensity * 25
        _collaboration_scores[step] = np.clip(base_collab + np.random.normal(0,2) ,5, 95)

        if num_team_members > 0 : 
            for i in range(num_team_members):
                # Only update positions if they exist and assigned_zone is valid for this worker
                if i < len(worker_assigned_zone) and i < len(worker_current_x):
