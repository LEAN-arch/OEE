...
Provides a professional, interactive UI with actionable visualizations, clear metrics, and company branding.
"""
...
# Custom CSS for professional dark theme (no pastels)
st.markdown("""
    <style>
        .main { 
        ...
        .stSelectbox, .stSlider, .stMultiSelect, .stFileUploader { 
...
with st.sidebar:
    ...
    if st.button("Help", key="help_button"):
        st.markdown("""
            ...
            - **Well-Being**: Worker Well-Being Index with actionable alerts (e.g., recommend breaks).
...
if st.session_state.simulation_results:
    ...
    with tabs[0]:
        ...
        # Composite line chart for landing page
        fig = go.Figure()
        minutes = [i * 2 for i in range(len(efficiency_metrics_df))]
    ...
    with tabs[9]:
        ...
        st.markdown("""
            ### Metric Definitions
            - **Task Compliance Score**: Percentage of tasks completed correctly (0–100%), based on adherence to protocols.
            - **Collaboration Proximity Index**: Percentage of workers within 5 meters of colleagues in the same zone, indicating teamwork effectiveness (0–100%).
            - **Operational Recovery Score**: Ability to maintain production output after disruptions (0–100%).
            - **Worker Well-Being Index**: Measure of worker fatigue, stress, and satisfaction (0–100%), influenced by breaks and workload.
            - **Psychological Safety Score**: Workers’ comfort in reporting issues, collaborating, and taking initiative without fear of repercussions (0–100%), based on surveys and interactions.
            ...
            - **OEE (Overall Equipment Effectiveness)**: Combined metric of uptime, throughput, and quality (0–100%).
            ...
            - **Feedback Impact**: Estimated improvement in well-being or team cohesion from initiatives (e.g., breaks or recognition).
        """)
        st.markdown("""
            ### Terms
            - **Disruption**: An event (e.g., equipment failure, supply delay) causing a temporary drop in performance metrics.
            - **Team Initiative**: Strategies like "More frequent breaks" (improves well-being) or "Team recognition" (boosts psychological safety).
            - **Anomaly**: A statistically significant deviation (z-score > 2.0) in metrics, indicating potential issues.
        """)
