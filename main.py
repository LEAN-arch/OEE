...
# These palettes and colors should be used in visualizations.py
...
ACCESSIBLE_CATEGORICAL_PALETTE = [
    ...
    # "#999999",  # Gray - good for less important elements or if black is too stark
]

# High contrast palette for categorical data
HIGH_CONTRAST_CATEGORICAL_PALETTE = [
    "#FFFF00",  # Bright Yellow
    "#0057FF",  # Bright Blue (adjusted from #0000FF for better perception on dark bg)
    "#FF00FF",  # Magenta
    "#00FFFF",  # Cyan
    "#FFFFFF",  # White
    "#39FF14",  # Neon Green (was #00FF00)
...
# For high contrast sequential, consider simpler scales or starker versions:
HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES = ["OrRd", "Greys", "YlOrBr"]


# Semantic colors for indicators (e.g., gauges, positive/negative trends)
# Ensure these provide good contrast with background and text (typically light text on dark theme)
# These are used in CSS alerts below and should be used for similar semantic meaning in plots.
COLOR_CRITICAL_RED = "#E53E3E"     # Tailwind Red 500
COLOR_WARNING_AMBER = "#F59E0B"    # Tailwind Amber 500
COLOR_POSITIVE_GREEN = "#10B981"   # Tailwind Emerald 500
COLOR_INFO_BLUE = "#3B82F6"        # Tailwind Blue 500
COLOR_NEUTRAL_GRAY = "#6B7280"     # Tailwind Gray 500
COLOR_ACCENT_INDIGO = "#4F46E5"    # Existing primary accent, good for highlights, needles

# --- Guidance for visualizations.py ---
# When implementing plotting functions in visualizations.py:
# 1. All plotting functions should accept the `high_contrast_setting` boolean.
# 2. Based on `high_contrast_setting`, choose colors from the appropriate palettes above.
#    - For categorical plots (lines, bars, pie charts):
#      `colors = HIGH_CONTRAST_CATEGORICAL_PALETTE if high_contrast_setting else ACCESSIBLE_CATEGORICAL_PALETTE`
#      Pass this to `color_discrete_sequence` in Plotly figures or map to individual traces.
#    - For sequential plots (heatmaps, continuous color scales):
#      `colorscale = random.choice(HIGH_CONTRAST_SEQUENTIAL_PLOTLY_SCALES) if high_contrast_setting else random.choice(ACCESSIBLE_SEQUENTIAL_PLOTLY_SCALES)`
#      Pass this string to `colorscale` in Plotly heatmaps, choropleths, etc.
# 3. For semantic coloring (e.g., gauge sections, up/down indicators):
#    Use `COLOR_POSITIVE_GREEN`, `COLOR_CRITICAL_RED`, `COLOR_WARNING_AMBER`, `COLOR_NEUTRAL_GRAY`.
# 4. Ensure plot backgrounds are transparent or match the theme:
#    `fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')`
# 5. Set plot font colors to be light and contrast well with the dark theme background:
#    `fig.update_layout(font_color="#EAEAEA", title_font_color="#FFFFFF", legend_font_color="#D0D0D0")`
#    Adjust specific tickfont, axis title font colors as needed.
# 6. Beyond color, use other visual distinctions:
#    - Different marker shapes for scatter plots.
#    - Varied line styles (solid, dash, dot) for line charts.
#    - Patterns for bar or pie charts (though Plotly's support for this is limited, direct labeling is better).
#    - Clear legends and direct data labeling where appropriate.
# 7. Test plots using color blindness simulation tools (e.g., browser extensions like "Colorblindly", online tools like Coblis).
# --- End Guidance ---


# CSS for Dark Theme (with accessibility improvements for alerts)
st.markdown(f"""
    <style>
        /* Base Styles */
        ...
        [data-testid="stSidebar"] .stButton>button {{ background-color: #34D399; /* Medium Aquamarine - generally okay */ width: 100%; margin-bottom: 0.5rem; }}
...
def main():
    ...
    with tabs[0]:
        ...
        if st.session_state.simulation_results:
            ...
            try:
                # Pass semantic colors for gauge thresholds
                summary_figs = plot_key_metrics_summary(
                    compliance, proximity, wellbeing, downtime_total_overview,
                    current_high_contrast_setting,
                    color_positive=COLOR_POSITIVE_GREEN,
                    color_neutral=COLOR_NEUTRAL_GRAY, # Or COLOR_WARNING_AMBER if more appropriate for mid-range
    ...
    with tabs[2]:
        ...
        if st.session_state.simulation_results:
            ...
            with st.container(border=True):
                ...
                if insights_count == 0: st.markdown(f"<p class='insight-text' style='color: {COLOR_POSITIVE_GREEN};'>✅ No specific well-being alerts triggered in the selected period.</p>", unsafe_allow_html=True) # Using COLOR_POSITIVE_GREEN
