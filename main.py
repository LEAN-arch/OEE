...
def plot_worker_distribution(df, facility_size, config, use_3d=False):
    ...
    try:
        # Validate DataFrame
        required_columns = ['x', 'y', 'step', 'worker_id', 'zone']
        if not all(col in df.columns for col in required_columns):
            missing_cols = [col for col in required_columns if col not in df.columns]
            logger.error(f"DataFrame missing required columns: {missing_cols}")
            raise ValueError(f"DataFrame missing required columns: {missing_cols}")
        
        # Default color map if ZONE_COLORS is missing
        default_colors = {
            'default': '#3B82F6',
            'unknown': '#EF4444'
        }
        zone_colors = config.get('ZONE_COLORS', default_colors)
        if not zone_colors:
            logger.warning("ZONE_COLORS not found in config. Using default colors.")
        
        # Ensure all zones in DataFrame have a color
        unique_zones = df['zone'].unique()
        for zone in unique_zones:
            if zone not in zone_colors:
                zone_colors[zone] = default_colors['unknown']
                logger.warning(f"Zone '{zone}' not in ZONE_COLORS. Assigned default color: {default_colors['unknown']}")
